...
def main():
	...
	parser.add_argument('-v', '--sample_vcf', dest = 'sample_vcf', action = 'store', type = str, default = None, help = "Sample VCF from GATK Unified Genotyper")
	parser.add_argument('-l', '--loess_span', dest = 'loess_span', action = 'store', type = float, default = .1, help = "Loess span")
	parser.add_argument('-d', '--d_yaxis', dest = 'd_yaxis', action = 'store', type = float, default = 1, help = "y-axis upper limit for dot plot")  
	parser.add_argument('-y', '--h_yaxis', dest = 'h_yaxis', action = 'store', type = int, default = 0, help = "y-axis upper limit for histogram plot")   
	parser.add_argument('-c', '--points_color', dest = 'points_color', action = 'store', type = str, default = "gray27", help = "Color for data points") 
	parser.add_argument('-k', '--loess_color', dest = 'loess_color', action = 'store', type = str, default = "red", help = "Color for loess regression line")        
	parser.add_argument('-z', '--standardize', dest = 'standardize', default= 'true', help = "Standardize X-axis")
	parser.add_argument('-b', '--break_file', dest = 'break_file', action = 'store', type = str, default = 'C.elegans', help = "File defining the breaks per chromosome")
	parser.add_argument('-x', '--bin_size', dest = 'bin_size', action = 'store', type = int, default = 1000000, help = "Size of histogram bins, default is 1mb")
	parser.add_argument('-n', '--normalize_bins', dest = 'normalize_bins', default= 'true', help = "Normalize histograms")


	parser.add_argument('-o', '--output', dest = 'output', action = 'store', type = str, default = None, help = "Output file name")
	parser.add_argument('-s', '--location_plot_output', dest = 'location_plot_output', action = 'store', type = str, default = "SNP_Mapping_Plot.pdf", help = "Output file name of SNP plots by chromosomal location")

	args = parser.parse_args()

	vcf_info = parse_vcf(sample_vcf = args.sample_vcf)

	output_vcf_info(output = args.output, vcf_info = vcf_info)
	
	rounded_bin_size = int(round((float(args.bin_size) / 1000000), 1) * 1000000)
	
	normalized_histogram_bins_per_mb = calculate_normalized_histogram_bins_per_xbase(vcf_info = vcf_info, xbase = rounded_bin_size, normalize_bins = args.normalize_bins)
	...
	normalized_histogram_bins_per_5kb = calculate_normalized_histogram_bins_per_xbase(vcf_info = vcf_info, xbase = (rounded_bin_size / 2), normalize_bins = args.normalize_bins)
	...
	break_dict = parse_breaks(break_file = args.break_file)

	output_scatter_plots_by_location(location_plot_output = args.location_plot_output, vcf_info = vcf_info, loess_span=args.loess_span, d_yaxis=args.d_yaxis, h_yaxis=args.h_yaxis, points_color=args.points_color, loess_color=args.loess_color, standardize =args.standardize, normalized_hist_per_mb = normalized_histogram_bins_per_mb, normalized_hist_per_5kb = normalized_histogram_bins_per_5kb, breaks = break_dict, rounded_bin_size = rounded_bin_size, max_y_hist_overall = max_y_hist_overall)
...
def parse_breaks(break_file = None):
	if break_file == 'C.elegans':
		...
		for row in reader:
			...
			#Brachy
			chromosome = re.sub("Bd", "", chromosome, flags = re.IGNORECASE)
			chromosome = re.sub("bd", "", chromosome, flags = re.IGNORECASE)
...
def plot_data(chr_dict =  None, hist_dict_mb = None, hist_dict_5kb = None, chr = "", x_label = "", divide_position = False, draw_secondary_grid_lines = False, loess_span=None, d_yaxis=None, h_yaxis=None, points_color="", loess_color="", breaks = None, standardize= None, max_breaks = 1, break_unit = 1):
	...
	if (standardize=='true'):  
		r("plot(x, y, cex=0.60, xlim=c(0," + max_break_str + "), main='LG " + chr + "', xlab= '" + x_label + "', ylim = c(0, %f " %d_yaxis + "), ylab='Percentage of mapping strain alleles/total reads (at SNP positions)', pch=10, col='"+ points_color +"')")
		...
		r("plot(x, y, cex=0.60, main='LG " + chr + " ', xlab= '" + x_label + "', ylim = c(0, %f " %d_yaxis + "), ylab='Percentage of mapping strain alleles/total reads (at SNP positions)', pch=10, col='"+ points_color +"')")
		r("lines(loess.smooth(x, y, span = %f "%loess_span + "), lwd=5, col='"+ loess_color +"')")    
...
def parse_vcf(sample_vcf = None):
	...
	for row in reader:
		...
		#Brachy
		chromosome = re.sub("Bd", "", chromosome, flags = re.IGNORECASE)
		chromosome = re.sub("bd_", "", chromosome, flags = re.IGNORECASE)
